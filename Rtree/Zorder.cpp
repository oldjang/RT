	...
	for (int i = bitNum - 1; i > 0; i--) {
		for (l = 0; l < dim; l++) {
			...
			if (l == 0) {
				...
				zdr[i * rowWidth + pos] = (zdr[i * rowWidth + pos] << 1)
						| (v[l] & 1);
			}
			v[l] >>= 1;
		}
...
bool compZorderByCoord(const int* v1, const int* v2) {
	return compZorderByCoord(v1, v2, DIM);
}

bool compZorderByCoord(const int* v1, const int* v2, int dim) {
	int pos = 0;
	int curMax = 0;
	int temp = 0;
...
bool compZorderByCoord(const vector<int>& v1, const vector<int>& v2, int dim) {
	int pos = 0;
	int curMax = 0;
	int temp = 0;
